...
public class TalkActivity extends AppCompatActivity {
    ...
    public void refresh() { startActivityForResult(new Intent(this, MeetingScheduleActivity.class), 1001); }
            ...
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                if(task.isSuccessful()) {
                    if(task.getResult().exists()) {
                        ...
                        FirebaseFirestore.getInstance().collection("meetings").document(title).collection("chat").document(currentUser.getNumber_update()).set(chat).addOnSuccessListener(new OnSuccessListener<Void>() {
                            @Override
                            public void onSuccess(Void aVoid) {
                                ...
                                FirebaseFirestore.getInstance().runTransaction(new Transaction.Function<Void>() {
                                    ...
                                    public Void apply(Transaction transaction) {
                                        transaction.update(meetings, "chatnum", chatnum + 1);
                                        return null;
                                    }

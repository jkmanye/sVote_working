...
public class VoteManageActivity extends AppCompatActivity {
            ...
            documentReference.get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
                ...
                public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                    if (task.isSuccessful()) {
                        ...
                        Lists = (HashMap<String, String>) documentSnapshot.get("Lists");
                        answer = (HashMap<String, Long>) documentSnapshot.get("answer");
    ...
    public void showPieChart(View view) {
        if(isUpdated) {
            if(t) {
                Intent i = new Intent(this, VotePieChartActivity.class);
                i.putExtra("size", answer);
                i.putExtra("lists", Lists);
                i.putExtra("total", total);
                i.putExtra("title", title);
                startActivityForResult(i, 1001);
            } else {
                Toast.makeText(getApplicationContext(), "먼저 집계를 한 후 그래프를 볼 수 있습니다.", Toast.LENGTH_LONG).show();
            }
        } else {
            Toast.makeText(getApplicationContext(), "먼저 투표명을 입력하여 검색한 후 그래프를 볼 수 있습니다.", Toast.LENGTH_LONG).show();
        }
    }
}